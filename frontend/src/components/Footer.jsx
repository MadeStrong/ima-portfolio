...
const Footer = () => {
  const { navigation, socialLinks, settings } = useData();
  ...
  return (
    <footer className="bg-[#0B0B0B] border-t border-[#1F1F1F]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            ...
            <div className="flex items-center gap-4">
              {socialLinks.map((link) => (
                <a
                  ...
                  className="w-10 h-10 rounded-full bg-[#1F1F1F] flex items-center justify-center text-[#B3B3B3] hover:bg-[#E10600] hover:text-white transition-all"
        ...
        {/* Bottom Bar */}
        <div className="mt-16 pt-8 border-t border-[#1F1F1F] flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-[#B3B3B3] text-sm">
            {settings?.footer_text || "© 2025 IMA. All rights reserved."}
          </p>
          <div className="flex items-center gap-6 text-sm text-[#B3B3B3]">
            <Link to="/admin/login" className="hover:text-white transition-colors" data-testid="footer-admin-link">
              Admin
            </Link>
          </div>
        </div>
